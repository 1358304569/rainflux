...
class my_gui():
    ...
    def set_init_window(self):
        ...
        # point，应力应变系数k，bins参数
        self.point_label = Label(self.init_window_name, text="Point:", bg='lightblue')
        self.point_label.place(x=50, y=130, width=70, height=30)
        self.point_text = Entry(self.init_window_name)
        self.point_text.insert(END, "1")  # 默认从第一列开始
        self.point_text.place(x=130, y=130, width=70, height=30)

        self.k_label = Label(self.init_window_name, text="系数K:", bg='lightblue')
        self.k_label.place(x=220, y=130, width=70, height=30)
        self.k_text = Entry(self.init_window_name)
        self.k_text.insert(END, "0.071")  # 默认
        self.k_text.place(x=300, y=130, width=70, height=30)

        self.bins_label = Label(self.init_window_name, text="Bins:", bg='lightblue')
        self.bins_label.place(x=390, y=130, width=70, height=30)
        self.bins_text = Entry(self.init_window_name)
        self.bins_text.insert(END, "16")  # 默认分箱为16
        self.bins_text.place(x=470, y=130, width=70, height=30)
    ...
    def start_rainflow_main(self):
        ...
        # 带有K的
        MyRainflow.main(self.file_text['text'], int(self.point_text.get()), float(self.k_text.get())
            , int(self.bins_text.get()), int(self.fs1_text.get()), int(self.noverlap_text.get())
            , int(self.nfft_text.get()), int(self.fs2_text.get()), int(self.cut_freq_text.get())
            , int(self.order_text.get()))


        # 不带K 的
        # MyRainflow.main(self.file_text['text'], int(self.point_text.get())
        #     , int(self.bins_text.get()), int(self.fs1_text.get()), int(self.noverlap_text.get())
        #     , int(self.nfft_text.get()), int(self.fs2_text.get()), int(self.cut_freq_text.get())
        #     , int(self.order_text.get()))
        #
